...
import mplcursors  # For interactive cursor on plot

# Load the data from CSV
data = pd.read_csv('C:/Users/legen/OneDrive/YG/Excel/STCOKDATA/AMZN.csv')
...
if len(scaled_data) < time_step:
    ...
    if time_step < 1:
        raise ValueError("Dataset is too small for the time_step. Please use a larger dataset.")
else:
...
mplcursors.cursor(hover=True).connect("add", lambda sel: sel.annotation.set_text(
    f'Price: ${sel.target[1]:.2f}'))  # Only display the stock price, no month info
...
# Reverse scaling for the actual predicted prices
predicted_future_prices = scaler.inverse_transform(np.array(predicted_future_prices).reshape(-1, 1))

# Plotting the future predictions with specific month labels
...
# Label the x-axis with specific month labels
future_month_labels = ["October 2024", "November 2024", "December 2024", 
                       "January 2025", "February 2025", "March 2025"]
plt.xticks(ticks=future_dates, labels=future_month_labels, rotation=45)
