...
# Load the data from CSV (for Google)
data = pd.read_csv('C:/Users/legen/OneDrive/YG/Excel/STCOKDATA/GOOGL.csv')  # Update with Google stock data file path
...
num_ticks = min(12, total_points)  # Max of 12 months
...
mplcursors.cursor(hover=True).connect("add", lambda sel: sel.annotation.set_text(
    f'Price: ${sel.target[1]:.2f}'))  # Show only Y values (Price) on hover

plt.xlabel('Time (Months)')
plt.ylabel('Stock Price (USD)')
plt.title('Google Stock Price Prediction\nLSTM Model Results')  # Title updated for Google
plt.legend()
plt.show()
...
plt.xlabel('Time (Months)')
plt.ylabel('Stock Price (USD)')
plt.title('Google Stock Price Prediction\nFuture 6 Months Forecast')  # Title updated for Google
plt.legend()
plt.show()
